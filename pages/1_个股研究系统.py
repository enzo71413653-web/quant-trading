...
中/美/韩/日 皆可；每次联网实时拉取，失败回退本地缓存。富可视化用 Plotly。
"""
import sys
import pathlib
...
if source == "cache":
    st.caption("⚠️ 联网失败，用本地缓存（非实时）。点侧栏 🔄 重试。")
else:
...
with tab1:
    ...
    c1 = st.columns(4)
    c1[0].metric("最新价", f"{close.iloc[-1]:.2f}", f"{chg:.2%}")
    c1[1].metric("累计收益", f"{(1 + returns).prod() - 1:.1%}")
    c1[2].metric("最大回撤", f"{qs.stats.max_drawdown(returns):.1%}")
    c1[3].metric("夏普", f"{qs.stats.sharpe(returns):.2f}")
    try:
        c2 = st.columns(4)
        c2[0].metric("年化收益", f"{qs.stats.cagr(returns):.1%}")
        c2[1].metric("年化波动", f"{qs.stats.volatility(returns):.1%}")
        c2[2].metric("索提诺", f"{qs.stats.sortino(returns):.2f}")
        c2[3].metric("卡玛", f"{qs.stats.calmar(returns):.2f}")
        hi52 = close.iloc[-252:].max() if len(close) >= 252 else close.max()
        c3 = st.columns(4)
        c3[0].metric("当前 RSI(14)", f"{_rsi(close).iloc[-1]:.0f}")
        c3[1].metric("距52周高", f"{close.iloc[-1] / hi52 - 1:.1%}")
        c3[2].metric("近1月", f"{close.iloc[-1] / close.iloc[-21] - 1:.1%}" if len(close) > 21 else "-")
        c3[3].metric("日胜率", f"{qs.stats.win_rate(returns):.1%}")
    except Exception:
        pass
    ...
    fig = make_subplots(rows=4, cols=1, shared_xaxes=True,
                        ...
                        subplot_titles=("K线 + 均线", "成交量", "RSI(14)", "MACD"))
    ...
    for n, col in [(20, "#f5c518"), (60, "#42a5f5")]:
        fig.add_trace(go.Scatter(x=df.index, y=close.rolling(n).mean(), name=f"MA{n}",
                                 line=dict(width=1, color=col)), row=1, col=1)
    fig.add_trace(go.Bar(x=df.index, y=df["Volume"], name="成交量", marker_color="#888"), row=2, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=_rsi(close), name="RSI", line=dict(color="#ab47bc")), row=3, col=1)
    fig.add_hline(y=70, row=3, col=1, line=dict(dash="dot", color="#ef5350"))
    fig.add_hline(y=30, row=3, col=1, line=dict(dash="dot", color="#26a69a"))
    fig.add_trace(go.Bar(x=df.index, y=hist, name="MACD柱",
                         marker_color=["#ef5350" if v >= 0 else "#26a69a" for v in hist]), row=4, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=dif, name="DIF", line=dict(color="#f5c518", width=1)), row=4, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=dea, name="DEA", line=dict(color="#42a5f5", width=1)), row=4, col=1)
    fig.update_layout(height=820, template="plotly_dark", xaxis_rangeslider_visible=False,
                      margin=dict(t=30, b=10, l=10, r=10), legend=dict(orientation="h", y=1.03))
...
with tab2:
    ...
    if closes:
        ...
        if not price.empty:
            ...
            f2.update_layout(height=440, template="plotly_dark", title="归一净值（起点=100，看谁跑得快）",
